...
# Function to map emotions to a reduced set
def map_emotions_to_reduced_set(df, emotion_column='emotional_state'):
    ...
    if len(unknown_emotions) > 0:
        logger.warning(f"Found unmapped emotions: {unknown_emotions}")
    ...
    def __getitem__(self, idx):
        ...
        for i, (_, row) in enumerate(batch_data.iterrows()):
            ...
            try:
                ...
                # Debug info for first batch if idx is 0
                if idx == 0 and i == 0:
                    logger.debug(f"First batch emotion shape: {batch_emotions.shape}")
                    logger.debug(f"Emotion one-hot: {batch_emotions[i]}")
...
class DogEmotionTrainer:
    ...
    def setup_data_generators(self, img_size=(224, 224), batch_size=32, use_reduced_emotions=False):
        """
        ...
        Args:
            img_size (tuple): Target image size (height, width)
            batch_size (int): Batch size
            use_reduced_emotions (bool): Whether to use the reduced emotion set
            
        Returns:
            tuple: (train_generator, val_generator, behavior_cols)
        """
        # Load annotations with appropriate emotion set
        if use_reduced_emotions:
            train_df = self.load_annotations_with_reduced_emotions('train')
            val_df = self.load_annotations_with_reduced_emotions('val')
            emotion_column = 'reduced_emotional_state'
        else:
            train_df = self.load_annotations('train')
            val_df = self.load_annotations('val')
            emotion_column = 'emotional_state'
        ...
        # Print unique emotions for debugging
        logger.info(f"Training data emotions: {train_df[emotion_column].unique()}")
        logger.info(f"Validation data emotions: {val_df[emotion_column].unique()}")
        ...
        # Make sure validation uses same emotion mapping as training
        val_generator = DogEmotionDataGenerator(
            ...
            use_bbox=True,
            unknown_as_none=True,
            emotion_to_idx=train_generator.emotion_to_idx  # Use same mapping
        )
        
        # Print emotion mapping for verification
        logger.info("Using emotion mapping:")
        for emotion, idx in train_generator.emotion_to_idx.items():
            logger.info(f"  {emotion}: {idx}")
    ...
    def train_model(self, model_type='image', epochs=50, learning_rate=None, 
        ...
        Args:
            ...
            use_reduced_emotions (bool): Whether to use reduced emotion categories
        ...
        train_generator, val_generator, behavior_cols = self.setup_data_generators(
            ...
            batch_size=batch_size,
            use_reduced_emotions=use_reduced_emotions
        )
        ...
        # Set up callbacks
        model_name = f"dog_emotion_{model_type}"
        if use_reduced_emotions:
            model_name += "_reduced"
            
        callbacks, checkpoint_dir = self.setup_callbacks(model_name)
        ...
        with open(emotion_mapping_path, 'w') as f:
            json.dump(emotion_mapping, f, indent=2)
        logger.info(f"Saved emotion mapping to {emotion_mapping_path}")
        
        # Save behavior columns
        behavior_cols_path = os.path.join(checkpoint_dir, 'behavior_columns.json')
        with open(behavior_cols_path, 'w') as f:
        ...
        # Train the model
        logger.info(f"Starting training for {model_type}-based model with {num_emotions} emotions...")
        history = model.fit(
            train_generator,
        ...
        if fine_tune:
            ...
            ft_callbacks, _ = self.setup_callbacks(f"{model_name}_finetune")
        ...
        metadata = {
            ...
            'used_reduced_emotions': use_reduced_emotions,
    ...
    def evaluate_model(self, model, model_type='image', checkpoint_dir=None, use_reduced_emotions=False):
        """
        ...
        # Load test data with appropriate emotion set
        if use_reduced_emotions:
            test_df = self.load_annotations_with_reduced_emotions('test')
            emotion_column = 'reduced_emotional_state'
        else:
        ...
        if os.path.exists(emotion_mapping_path):
            with open(emotion_mapping_path, 'r') as f:
                ...
                # Create reversed mapping for evaluation
                idx_to_emotion = {int(idx): emotion for idx, emotion in emotion_mapping.items()}
                emotion_to_idx = {emotion: int(idx) for idx, emotion in emotion_mapping.items()}
        else:
            # Extract emotions from test data
            emotions = test_df[emotion_column].dropna().unique().tolist()
            emotion_to_idx = {emotion: i for i, emotion in enumerate(emotions)}
            idx_to_emotion = {i: emotion for i, emotion in enumerate(emotions)}
        
        # Print emotion mapping for verification
        logger.info(f"Evaluation using {len(emotions)} emotions: {emotions}")
        ...
        test_generator = DogEmotionDataGenerator(
            ...
            augment=False,
            use_bbox=True,
            emotion_to_idx=emotion_to_idx  # Use same mapping as in training
        )
        ...
        self.generate_classification_metrics(model, test_generator, model_type, checkpoint_dir, idx_to_emotion)
        
        return evaluation_dict
    
    def generate_classification_metrics(self, model, test_generator, model_type, checkpoint_dir, idx_to_emotion=None):
        """
        ...
        Args:
            ...
            idx_to_emotion (dict): Optional mapping from indices to emotion names
        """
        # Get predictions
        y_true = []
        y_pred = []
        ...
        # Get class names
        if idx_to_emotion is not None:
            class_names = [idx_to_emotion.get(i, f"Class {i}") for i in range(len(test_generator.emotions))]
        else:
        ...
        with np.errstate(divide='ignore', invalid='ignore'):
            ...
            cm_norm = np.nan_to_num(cm_norm)  # Replace NaN with zero and inf with large finite numbers
        ...
        metrics_path = os.path.join(
            checkpoint_dir,
            f"{model_name}_class_metrics.json"
        )
    ...
    def plot_training_history(self, history, model_type, checkpoint_dir=None, use_reduced_emotions=False):
        """
        ...
        Args:
            ...
            use_reduced_emotions (bool): Whether reduced emotions were used
        """
        if history is None:
        ...
        if 'lr' in history.history:
            ...
            lr_plot_path = os.path.join(
                checkpoint_dir,
                f"{model_name}_learning_rate.png"
            )
    ...
    def analyze_model_performance(self, checkpoint_dir):
        ...
        # Sort by F1 score (same order as previous plot)
        ...
        # Precision chart
        bars1 = ax[0].barh(emotions, precision, color='lightblue')
        ax[0].set_title('Precision by Emotion')
        ax[0].set_xlim(0, 1)
        ax[0].grid(axis='x', linestyle='--', alpha=0.7)
        ax[0].set_xlabel('Precision')
        
        # Recall chart
        bars2 = ax[1].barh(emotions, recall, color='lightgreen')
        ax[1].set_title('Recall by Emotion')
        ax[1].set_xlim(0, 1)
        ...
        pr_path = os.path.join(checkpoint_dir, "precision_recall_analysis.png")
        plt.savefig(pr_path, dpi=300, bbox_inches='tight')
        plt.close()
        
        logger.info(f"Precision-recall analysis saved to {pr_path}")
...
if __name__ == "__main__":
    ...
    if IN_NOTEBOOK:
        ...
        print(f"Using reduced emotions: {use_reduced_emotions}")
        ...
        parser = argparse.ArgumentParser(description="Dog Emotion Model Training")
        ...
        parser.add_argument("--reduced-emotions", action="store_true", 
                            help="Use reduced emotion categories")
        ...
        model, history, checkpoint_dir = trainer.train_model(
            ...
            use_reduced_emotions=args.reduced_emotions
        )
