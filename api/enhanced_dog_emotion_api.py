# enhanced_api.py - Windows Local Development Version - FIXED
...
def load_model():
    """Load the trained model - supports both .keras and SavedModel formats"""
    global model, model_type, infer_func, input_shapes
    
    try:
        # Define possible model locations
        model_paths = [
            # SavedModel format (from export script)
            "cloud_export/saved_model",
            "./cloud_export/saved_model", 
            "saved_model",
            "./saved_model",
            ...
            if os.path.exists(model_path):
                ...
                try:
                    ...
                    if os.path.isdir(model_path) and "saved_model.pb" in os.listdir(model_path):
                        print(f"📁 Loading SavedModel format...")
                    ...
                    # Try loading as Keras model (.keras or .h5)
                    elif model_path.endswith(('.keras', '.h5')):
                        print(f"📄 Loading Keras model format...")
...
def predict_with_savedmodel(img_array):
    """Make prediction using SavedModel format"""
    try:
        # Get input names and create appropriate inputs
        ...
        for name in input_names:
            if 'image' in name.lower():
                ...
                # Create behavior input based on expected shape
...
def predict_with_keras(img_array):
    """Make prediction using Keras model format"""
    try:
        # Create dummy behavior input 
        behavior_input = np.zeros((1, 64))  # Adjust size as needed
        
        # Prepare inputs for model
        inputs = {
            'image_input': img_array.astype(np.float32),
        ...
        return emotion_probs[0], confidence[0][0]
        
    except Exception as e:
        print(f"❌ Keras prediction error: {e}")
        # Try simple prediction if named inputs don't work
        try:
            predictions = model.predict([img_array, np.zeros((1, 64))])
...
async def predict_emotion(request: ImageRequest):
    ...
    try:
        ...
        if model_type == "savedmodel":
            ...
            emotion_probs, confidence_score = predict_with_keras(img_array)
        else:  # dummy model
